    ...
    load=[
        ...
        selectinload(m.User.teams).options(
            joinedload(m.TeamMember.team, innerjoin=True).options(
                load_only(m.Team.name),
                # Loaded (not just team name/id) so POS store-access guards can check
                # `membership.team.stores` without triggering a lazy-load on the
                # already-detached-from-request-context connection.user object.
                selectinload(m.Team.stores).options(load_only(m.Store.name)),
            ),
            joinedload(m.TeamMember.assigned_store, innerjoin=False).options(load_only(m.Store.name)),
        ),
    ],
    uniquify=True,
